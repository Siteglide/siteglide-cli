 ...
 *
 * Written to `.siteglide/user/sync/current-conflict.json` while sync is waiting for
...
const { clearConflictLog, readConflictLog, updateConflictLog } = require('./remoteCheckConflictLog');
const { rel, migrateLegacySiteglideLayout } = require('./siteglidePaths');

const SCHEMA_VERSION = 1;
const RELATIVE_PATH = rel.syncCurrentConflict;
 ...
 * @param {string} [opts.kind]
	 * @param {string[]} [opts.wavePaths]
	 * @param {string} [opts.cwd]
 * @param {string} [opts.status]
 * @param {boolean} [opts.syncPaused]
...
function writeSyncCurrentConflict(opts) {
	...
	const status = opts.status || 'awaiting_user_decision';
	const syncPaused = opts.syncPaused !== false;
	...
	const body = {
		...
		status,
		syncPaused,
		awaitingUserDecision: status === 'awaiting_user_decision',
		userDecision: null,
	...
	if (Array.isArray(opts.wavePaths) && opts.wavePaths.length > 0) {
		body.wavePaths = opts.wavePaths.map(normalizeConflictPath).filter(Boolean);
	}
...
/**
 * Clear live sync conflict + per-env remote-check log (MCP remote_check_status).
 * @param {string} environment
 * @param {string} [cwd]
 */
function clearSyncConflictRecords(environment, cwd = process.cwd()) {
...
/**
 * Clear agent conflict records when a path no longer needs a sync prompt.
 * @param {string} environment
 * @param {string} pathQuery physical or local path
 * @param {string} [cwd]
 * @returns {boolean} whether anything was cleared
 */
function clearSyncConflictForPath(environment, pathQuery, cwd = process.cwd()) {
	let cleared = false;
	const current = readSyncCurrentConflict(cwd);
	if (current && conflictMatchesPath(current, pathQuery)) {
		clearSyncCurrentConflict(cwd);
		cleared = true;
	}
	const env = environment || (current && current.environment);
	if (env) {
		const log = readConflictLog(env, cwd);
		if (log && log.command === 'sync' && conflictMatchesPath(log, pathQuery)) {
			clearConflictLog(env, cwd);
			cleared = true;
		}
	}
	return cleared;
...
function resolveSyncCurrentConflict(decision, cwd = process.cwd()) {
	...
	if (decision === 'merge_first') {
		...
		const body = {
			...
			agentGuidance:
				'Merge-first sync is in progress. Sync is not waiting for a prompt on this file.'
		};
		fs.writeFileSync(filePath, `${JSON.stringify(body, null, 2)}\n`, 'utf8');
		if (existing.environment) {
			updateConflictLog(existing.environment, {
				status: 'merge_in_progress',
				awaitingUserDecision: false,
				userDecision: 'merge_first',
				resolvedAt: body.resolvedAt,
				agentGuidance: body.agentGuidance
			}, cwd);
		}
		return;
	}
...
/**
 * Update status fields on the current sync conflict record (e.g. waiting for git).
 * @param {object} opts
 * @param {string} opts.status
 * @param {boolean} [opts.syncPaused]
 * @param {string} [opts.agentGuidance]
 * @param {string} [cwd]
 */
function updateSyncCurrentConflictStatus(opts, cwd = process.cwd()) {
	const existing = readSyncCurrentConflict(cwd);
	if (!existing) {
		return;
	}
	...
	const body = {
		...
		syncPaused,
		awaitingUserDecision: status === 'awaiting_user_decision',
		agentGuidance: opts.agentGuidance || existing.agentGuidance
	};
	if (status === 'waiting_for_git_resolution') {
		body.agentGuidance =
			opts.agentGuidance ||
			'Sync is paused while git conflict markers remain. ' +
			'The human must resolve merge conflicts and finish the merge commit before sync continues.';
	}
	const filePath = syncCurrentConflictPath(cwd);
	fs.writeFileSync(filePath, `${JSON.stringify(body, null, 2)}\n`, 'utf8');
}
