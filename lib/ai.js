...
const ensureMcpRegistered = (opts = {}) => {
	...
	if (added.length > 0) {
		logger.Info(`[pull] AI: Registered siteglide-mcp for: ${added.join(', ')}`);
	}
	if (updated.length > 0) {
		logger.Info(`[pull] AI: Updated siteglide-mcp launch path for: ${updated.join(', ')}`);
	}
	...
	if (added.length === 0 && updated.length === 0 && unchanged.length > 0 && errors.length === 0) {
		logger.Info('[pull] AI: siteglide-mcp already registered for supported IDEs');
	}
...
const ensureMcpIdeRules = (opts = {}) => {
	...
	try {
		...
		logger.Info(`[pull] AI: Wrote siteglide-mcp agent rules for: ${written.join(', ')}`);
	} catch (error) {
