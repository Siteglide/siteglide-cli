...
/** Project-local MCP config files written by pull (not Windsurf user-level config). */
...
/**
 * True when pull (or a prior pull) has written at least one project MCP config.
 * @param {string} [rootPath]
 * @returns {boolean}
 */
const hasProjectMcpConfig = (rootPath = process.cwd()) => {
	return PROJECT_MCP_CONFIG_RELATIVE_PATHS.some((relPath) => {
...
validate_code, siteglide_rules, siteglide_guide, envs_list, audience, graphql_exec, liquid_exec, logs_fetch.

Always run the Siteglide MCP audience tool when starting a new chat, if it is available.
...
module.exports = {
	SERVER_NAME,
	MCP_AGENT_GUIDANCE,
	PROJECT_MCP_CONFIG_RELATIVE_PATHS,
	hasProjectMcpConfig,
	buildMcpLaunchEntry,
