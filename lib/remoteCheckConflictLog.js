 ...
 *
 * Written under `.siteglide/user/remote-check/<environment>.json` so agents can call
...
const DIR_SEGMENTS = userSegments('remote-check');
...
function defaultRecommendedActions(opts = {}) {
	...
	if (opts.gitInitialized !== false) {
		actions.push({
			id: 'merge_first',
			priority: 1,
			summary: 'Pull remote site/modules/assets into local with git merge (conflict markers for AI), then save again to sync'
		});
	}
	actions.push({
		id: 'cancel',
		priority: opts.gitInitialized === false ? 1 : 2,
		summary: 'Cancel sync/deploy and review remote changes (pull when ready, or --skip-remote-check only if intentional)'
	});
	return actions;
}
...
/**
 * Patch fields on an existing per-env conflict log.
 * @param {string} environment
 * @param {object} patch
 * @param {string} [cwd]
 */
function updateConflictLog(environment, patch, cwd = process.cwd()) {
	const existing = readConflictLog(environment, cwd);
	if (!existing) {
		return null;
	}
	const filePath = conflictLogPath(environment, cwd);
