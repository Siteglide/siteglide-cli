 ...
 * Project `.siteglide-modules/pull.json` can add (`exclude`) or remove (`include`) names.
 */
const fs = require('fs-extra');
const path = require('path');
const logger = require('./logger');

const PULL_MODULES_CONFIG_DIR = '.siteglide-modules';
const PULL_MODULES_CONFIG_FILE = 'pull.json';
...
const defaultPullModulesConfigDocument = () => {
	return {
		usage: [
			'Siteglide CLI pull module filter for this project.',
			'',
			'By default, pull skips built-in Siteglide platform modules (Studio, ecommerce, blog, etc.).',
			'Use this file to adjust that list for your site:',
			'  exclude — add module machine names to skip (merged onto the built-in list)',
			'  include — remove module machine names from the skip list (they will be pulled)',
			'Neither key replaces the built-in list.',
			'',
			'Commit this file to git (do not gitignore .siteglide-modules/) so every developer',
			'pulling the same site downloads the same modules.',
			'',
			'Example — pull module_357 but also skip a custom module:',
			'  "include": ["module_357"],',
			'  "exclude": ["my_custom_module"]',
			'',
			'Use siteglide-cli pull -m <name> to pull a single module once, including ignored ones.'
		].join('\n'),
		include: [],
		exclude: []
	};
...
/**
 * Create `.siteglide-modules/pull.json` when missing. Never overwrites an existing file.
 *
 * @param {string} [rootPath]
...
const readPullModulesConfig = async (rootPath = process.cwd()) => {
	...
	try {
		...
		if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
			logger.Warn(`[pull] ${PULL_MODULES_CONFIG_RELATIVE_PATH} must be a JSON object; using built-in ignore list only`, { exit: false });
			return { include: [], exclude: [] };
		}
		return {
			include: normalizeModuleList(parsed.include),
			exclude: normalizeModuleList(parsed.exclude)
		};
	} catch (error) {
