 ...
 * Project `.siteglide/project/modules.json` — `pull_behaviour.include` / `exclude` adjust the skip list.
 ...
 * Pull always downloads public/ files only — module private/ trees never leave the instance.
 */
...
const PULL_MODULES_CONFIG_RELATIVE_PATH = path.join(...projectSegments('modules.json'));
...
/**
 * Sole module that distributes AI agent skills on pull. Skills live in
 * `public/assets/agents/` on the instance and merge into `./.agents/` locally.
 * No other Siteglide module ships SKILL.md files.
 */
const AGENTS_ONLY_PULL_MODULES = [
	'module_984'
];
 ...
 * @returns {boolean}
 */
const isAgentsOnlyPullModule = (moduleName) => {
	return AGENTS_ONLY_PULL_MODULES.indexOf(moduleName) !== -1;
};
...
const defaultPullBehaviour = () => {
	return {
		usage: [
			'By default, pull skips Siteglide modules, but not marketplace or custom modules, whose public files will be downloaded (but may not necessarily be designed to be edited). Exclude will skip additional modules. include will pull additional modules which would otherwise be skipped. We recommend committing this file to git so the team pulls the same modules.',
			'',
			'AI skills: only module_984 distributes skills (SKILL.md files), delivered as module assets during pull — not from module backup zips. Matching files merge into ./.agents at the project root (read-only; nothing under ./modules/module_984/). Exclude module_984 if you do not want skills locally. Do not use --ignore-assets if you need skills.',
			'',
			'Examples:',
			'  "include": ["module_357"],',
...
/**
 * Create `.siteglide/project/modules.json` when missing. Never overwrites an existing file.
 *
 * @param {string} [rootPath]
