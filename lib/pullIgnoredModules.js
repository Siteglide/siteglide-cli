 ...
 * Project `.siteglide/cli-settings/modules.json` can add (`exclude`) or remove (`include`) names.
 *
...
const defaultPullModulesConfigDocument = () => {
	return {
		usage: [
			'Adjust pull\'s built-in module skip list: exclude adds names, include removes them — commit to git so the team pulls the same modules.',
			'',
			'Examples:',
			'  "include": ["module_357"],',
