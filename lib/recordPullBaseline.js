...
const { finalizePullBaseline } = require('./git/finalizePullBaseline');
...
function recordPullBaselineAfterPull(opts) {
	...
	if (head.ok && head.stdout) {
		try {
			finalizePullBaseline({ environment, cwd });
		} catch (err) {
			logger.Warn(`[pull] Could not finalize pull baseline: ${err.message}`, { exit: false });
