...
const _getAssets = async () => {
	const siteRoot = dir.getSiteRoot();
	const appAssets = siteRoot && fs.existsSync(`${siteRoot}/assets`)
		? await glob(`${siteRoot}/assets/**`, { onlyFiles: true, dot: true })
		: [];

	const moduleAssets = fs.existsSync(dir.MODULES)
		? await glob(`${dir.MODULES}/*/{public,private}/assets/**`, { onlyFiles: true, dot: true })
		: [];

	return [...appAssets, ...moduleAssets];
};
