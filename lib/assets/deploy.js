const packAssets = require('./packAssets'),
	ora = require('ora'),
...
const headExists = async (fileUrl) => {
	try {
...
const waitForUnpack = async fileUrl => {
	...
	do {
		...
		fileExists = await headExists(fileUrl);
...
const deployAssets = async (gateway) => {
	...
	try {
		...
		var stats = fs.statSync(assetsArchiveName);
		var fileSizeMb = stats.size / (1024 * 1024);
		if (fileSizeMb > 4999) {
			logger.Error('Total assets size must be smaller than 5GB');
		}
		const data = await presignUrl(remoteAssetsArchiveName, assetsArchiveName, gateway);
		const spinner = ora({ text: `Deploying assets to: ${gateway.url}`, stream: process.stdout }).start();
		logger.Debug(data);
