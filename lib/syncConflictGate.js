...
function createSyncConflictGate(opts) {
	 ...
	 */
	/**
	 * Block uploads while a wave conflict is being resolved.
	 */
	const blockUploads = () => {
		uploadsBlocked = true;
	};
	...
	/**
	 * Allow uploads after conflict resolution completes.
	 */
	const allowUploads = () => {
	 ...
	 * @returns {boolean}
	 */
	const areUploadsBlocked = () => uploadsBlocked;

	/**
	 * Safety net before push/delete — throws if uploads are blocked.
	 */
	const assertUploadAllowed = () => {
		if (uploadsBlocked) {
			throw new Error('sync upload blocked while conflict is unresolved');
		}
	};
