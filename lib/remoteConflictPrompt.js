...
async function promptRemoteConflict(opts) {
	...
	const logPath = writeConflictLog(opts.environment, {
		...
		agentGuidance: opts.command === 'sync'
			? 'Human must choose on the CLI sync prompt. Advise only; see also .siteglide/sync/current-conflict.json.'
			: undefined
	}, cwd);
	...
	if (opts.command === 'sync') {
		...
		if (git.repoInitialized) {
			choices.push({
				name: 'Commit, pull single file and merge — BUGGY - yml missing',
				value: 'merge_first',
				description:
					'This will commit your changes, create a temporary branch to store the remotely changed contents of the file and merge that branch into your current branch. You can resolve the merge conflicts manually, or with AI, and then continue.'
			});
		} else {
			choices.push({
				name: 'Commit, pull single file and merge (requires git) — BUGGY - yml missing',
				value: 'merge_first',
				description:
		...
		const answer = await selectChoice(
			`Detected a possible newer version of this file on remote site, updated ${when}. How would you like to proceed?`,
			choices
		);
...
async function promptDeployConfirm(opts) {
	...
	let statusClause;
	if (skipped) {
		statusClause = 'the remote last-updated check was skipped (--skip-remote-check)';
