...
const fetchSettings = (environment, program) => {
	...
	if (environment){
		logger.Warn(`No settings for ${environment} environment, please see siteglide add`);
	} else {
		logger.Warn('No environment specified, please pass environment for a command `siteglide add [environment] --email [email] --url [url] --token [token]`');
	}
	program.help();
};
...
const listEnvironments = () => {
	...
	if (list.length) {
		...
		logger.Error('No environments registered yet, please see siteglide add');
	}
};
