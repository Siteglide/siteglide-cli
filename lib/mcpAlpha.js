...
const ensureMcpOnPull = async (opts = {}) => {
	...
	logger.Info('[pull][mcp] starting MCP check');
	...
	if (!latestVersion) {
		...
		logMcpStep('MCP check complete (skipped)', overallStart, 'not published');
		return {
			skipped: true,
	...
	if (needsMcpInstall(installedVersion, latestVersion)) {
		if (interactive) {
			logMcpStepStart('waiting for MCP install confirmation');
			stepStart = Date.now();
			const answer = await Confirm(
				`Attempt Siteglide MCP install from ${DEFAULT_PACKAGE}@${latestVersion} on npm? (y/N) `
			);
			logMcpStep('MCP install confirmation', stepStart, isAffirmative(answer) ? 'yes' : 'no');
			if (isAffirmative(answer)) {
				...
				logger.Info('[pull] Skipping Siteglide MCP install');
			}
		} else {
			logger.Debug('[pull] MCP install skipped (non-interactive)');
		}
	} else {
	...
	if (afterConfig.configured) {
		logger.Info(`[pull] Siteglide MCP configured (${installedVersion})`);
