/**
 * Pull / deploy baseline state under `.siteglide/user/pull/<environment>.json`.
 *
...
const PULL_DIR_SEGMENTS = userSegments('pull');
const SYNC_RECORD_DEBOUNCE_MS = 500;

/** Floor is stored slightly above GraphQL updated_at so strict `>` comparison does not false-positive. */
const SYNC_FLOOR_BUFFER_MS = 1;
 ...
 * @returns {string}
 */
function syncRecordBatchKey(environment, cwd = process.cwd()) {
	return `${cwd}\0${environment}`;
}

/**
 * Pending debounced syncedAt for one path (Option B — visible before disk flush).
 * @param {string} environment
 * @param {string} normalizedPath
 * @param {string} [cwd]
 * @returns {string | null}
 */
function pendingSyncSyncedAt(environment, normalizedPath, cwd = process.cwd()) {
	const batch = pendingSyncRecords.get(syncRecordBatchKey(environment, cwd));
	if (!batch) {
		return null;
	}
	return batch.paths.get(normalizedPath) || null;
}

/**
 * Per-path sync floor from remote GraphQL updated_at (+ {@link SYNC_FLOOR_BUFFER_MS}).
 * @param {string} updatedAtIso
 * @returns {string | null}
 */
function floorFromRemoteUpdatedAt(updatedAtIso) {
	const ms = parseBaselineMs(updatedAtIso);
	if (ms == null) {
		return null;
	}
	return new Date(ms + SYNC_FLOOR_BUFFER_MS).toISOString();
}
...
function recordSyncPath(environment, physicalPath, opts = {}) {
	...
	const key = syncRecordBatchKey(environment, cwd);
...
function effectiveBaseline(environment, physicalPath, cwd = process.cwd()) {
	...
	if (baseline && baseline.lastSync && baseline.lastSync.paths) {
		syncAt = baseline.lastSync.paths[normalized] || null;
	}
	const pendingAt = pendingSyncSyncedAt(environment, normalized, cwd);
	if (pendingAt) {
		const diskMs = parseBaselineMs(syncAt);
		const pendingMs = parseBaselineMs(pendingAt);
		if (pendingMs != null && (diskMs == null || pendingMs > diskMs)) {
			syncAt = pendingAt;
		}
	}
	if (syncAt) {
