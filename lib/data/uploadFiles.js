...
const partnerPortalHost = () => process.env.PARTNER_PORTAL_HOST || 'https://partners.platform-os.com';
...
const askForSlot = (tmpFileName, filename) => {
	...
	const params = { fileName: s3FileName, contentLength: fs.statSync(tmpFileName)['size'], contentType: mime.lookup(tmpFileName) };

	return apiRequest({
		uri: `${process.env.DEPLOY_SERVICE_URL || process.env.SITEGLIDE_URL}/api/private/urls/presign-url`,
		headers: {
			token: process.env.SITEGLIDE_TOKEN,
			marketplace_domain: url.parse(process.env.SITEGLIDE_URL).hostname,
			marketplace_endpoint: partnerPortalHost()
		},
		qs: params,
		json: true
	}).then(body => ({ uploadUrl: body.url, accessUrl: url.parse(body.accessUrl).href }));
};
					...
					return askForSlot(item.url, filename)
						.then(data => {
							return new Promise((resolve) => {
								uploadFile(tmpFilename, data.uploadUrl).then(resolve(data.accessUrl));
							});
						})
