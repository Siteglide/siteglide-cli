...
const download = (url, maxRecursiveDepth) => {
	...
	return scrape({
		...
		urlFilter: (currentUrl) => {
			...
			if (currentUrl.includes('OrderRetrievev2.aspx')) return false;
			if (currentUrl.includes('BannerProcess.aspx')) return false;
		...
		sources: [
			...
			{ selector: 'img', attr: 'data-hidpi-src' }, //Support for Adobe Muse
...
const run = (program) => {
	return new Promise((resolve, reject) => {
		...
		spinner.start();
		if(program.muse){
			request({uri: `${normalizedUrl}/scripts/museconfig.js`, resolveWithFullResponse: true})
			.then((response) => {
				if(parseInt(response.statusCode/100)===2){
					try {
						let startPos = response.body.indexOf('paths:{');
						let endPos = response.body.indexOf('},');
						let jsonObj = JSON.parse(`{${response.body.slice(startPos,endPos+1).replace('paths','"paths"')}}`);
						Object.keys(jsonObj.paths).forEach(function(k){
							let museAsset = `${normalizedUrl}${jsonObj.paths[k].slice(0,1) === '/' ? jsonObj.paths[k] : '/'+jsonObj.paths[k]}`;
							if(museAsset){
								request({uri: museAsset, resolveWithFullResponse: true})
								.then((statusCheck) => {
									if(parseInt(statusCheck.statusCode/100)===2){
										download(museAsset, 0);
									}
								})
								.catch((e) => {})
							}
