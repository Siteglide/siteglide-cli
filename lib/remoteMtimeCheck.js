 ...
 */
async function fetchRemoteFileMtime(gateway, physicalPath, opts = {}) {
	...
	const data = await runGraph(
		gateway,
		buildFileMtimeQuery(mapped.queryField, physicalPath, { includeBody })
	);
	...
	if (includeBody) {
		...
		result.body = body != null ? String(body) : null;
	}
	return result;
}
 ...
 * @param {object} [opts.remoteMeta] pre-fetched result from fetchRemoteFileMtime
 * @returns {Promise<{ ok: true } | { ok: false, reason: string, conflict?: object, physicalPath: string }>}
 */
async function checkFile(opts) {
	const { gateway, environment, filePath, siteRoot, cwd, remoteMeta: cachedRemote } = opts;
	...
	const remote = cachedRemote != null
		? cachedRemote
		: await fetchRemoteFileMtime(gateway, physicalPath);
	if (remote.skipped) {
		return { ok: true, skipped: true, physicalPath };
	}
