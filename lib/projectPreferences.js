/**
 * Project-level preferences under `.siteglide/project/project-preferences.json`.
 *
...
const { joinProject, migrateLegacySiteglideLayout } = require('./siteglidePaths');
 ...
 * @param {string} [cwd]
 */
function migrateLegacyProjectPreferences(cwd = process.cwd()) {
	migrateLegacySiteglideLayout(cwd);
	const resolved = path.resolve(cwd);
	const projectPath = joinProject(resolved, 'project-preferences.json');
	if (fs.existsSync(projectPath)) {
		return;
	}
	const legacyPaths = [
		path.join(resolved, '.siteglide', 'project-preferences.json'),
		path.join(resolved, '.siteglide', 'user', 'project-preferences.json'),
		path.join(resolved, '.siteglide', 'IDE', 'project-preferences.json')
	];
	for (let i = 0; i < legacyPaths.length; i++) {
		...
		if (fs.existsSync(legacyPath)) {
			...
			try {
				fs.renameSync(legacyPath, projectPath);
			} catch {
				try {
					fs.copyFileSync(legacyPath, projectPath);
				} catch {
					// Best-effort; fresh write still works.
				}
			}
			return;
		}
...
function projectPreferencesPath(cwd = process.cwd()) {
	migrateLegacyProjectPreferences(cwd);
	return joinProject(cwd, 'project-preferences.json');
}
