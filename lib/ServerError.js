...
const ServerError = {
	...
	notFound: error => {
		...
		logger.Error(`[${error.statusCode}] ${errType==='Ping' ? 'Command' : errType} failed. Cannot find a site with the URL: ${error.options.headers.site}, please check your .siteglide-config file. ${error.options.headers.site.includes('.staging.oregon.platform-os.com') ? 'If your site is now live you will need to re-add the coniguration with the production URL' : null}`, { hideTimestamp: true });
	},
