...
const ServerError = {
	...
	notFound: error => {
		...
		logger.Error(`[${error.statusCode}] ${errType==='Ping' ? 'Command' : errType} failed. Cannot find a site with the URL: ${error.options.headers.site}, please check your .siteglide-config file. ${error.options.headers.site.includes('.staging.oregon.platform-os.com') || error.options.headers.site.includes('.staging-siteglide.com') ? 'If your site is now live you will need to re-add the configuration with the production URL' : ''}`, { hideTimestamp: true });
	},
